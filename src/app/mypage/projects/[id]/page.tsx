    ...
    fetch(`/api/projects/${id}`)
      ...
      .then((data) => {
        ...
        setForm({
          ...
          // 一覧からの autoGood 遷移：ステータスを good にプリセット
          ...(searchParams.get("autoGood") === "1" ? { status: "good" } : {}),
        });
      })
    ...
    fetch(`/api/projects/${id}`, {
      ...
      .then((data) => {
        if (data.id) {
          ...
          setTimeout(() => {
            router.push(`/mypage?tab=${targetTab(data.status)}`);
            router.refresh();
          }, 400);
        }
      })
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <div className="flex items-center justify-between">
        <Link href="/mypage" className="text-amber-700 hover:underline">← マイページ</Link>
      </div>
        ...
        <div className="space-y-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div>
              ...
              />
              {form.status === "good" && !form.departmentBudget && (
                <p className="mt-1 text-xs text-red-500">決定案件には部門予算が必要です</p>
              )}
            </div>
            <div>
        ...
        <div className="mt-6 flex gap-3">
          ...
          <button
            ...
            onClick={async () => {
              ...
              router.push(`/mypage?tab=${targetTab(form.status)}`);
              router.refresh();
            }}
          ...
          <Link
            href={`/mypage?tab=${targetTab(form.status)}`}
