  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      <div className="mb-2 flex items-center gap-4">
        ...
        <Link href="/admin?tab=proposal" className="text-sm text-stone-500 hover:underline">提案案件一覧</Link>
      </div>
              ...
              return (
                <div key={a.userId} className="flex items-center gap-2 rounded border border-stone-200 bg-white px-3 py-2 text-sm">
                  ...
                  <label className="flex items-center gap-1 text-xs text-stone-500">
                    ...
                    <select
                      value={a.involvement}
                      onChange={(e) => setAssignees((prev) => prev.map((x) =>
                  ...
                  <button
                    type="button"
                    onClick={() => setAssignees((prev) => prev.filter((x) => x.userId !== u.id))}
            ...
            {users.filter((u) => !assignees.some((a) => a.userId === u.id)).length > 0 && (
              <select
                ...
                onChange={(e) => {
                  ...
                  if (uid) {
                    setAssignees((prev) => [...prev, { userId: uid, involvement: "SUB" as Involvement }]);
